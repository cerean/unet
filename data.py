...
# altered from irrelevant inputs
ISBI = [30,512,512]
Unlabelled = [0,0,0]

COLOR_DICT = np.array([ISBI, Unlabelled])
...
def adjustData(img, mask, flag_multi_class, num_class):
    ...
    if(flag_multi_class):
        ...
        for i in range(num_class):
            ...
            #index = tf.where(mask == i)
            #index_mask = (index[0], index[1], index[2], np.zeros(len(index[0]), dtype = np.int64) + i) \
                #if (len(mask.shape) == 4) \
                #else (index[0], index[1], np.zeros(len(index[0]), dtype = np.int64) + i)
...
def trainDataNpy(image_path, mask_path, flag_multi_class = False, num_class = 2, 
